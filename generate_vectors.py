...
if __name__ == "__main__":
	...
	#summary_table = summary_table.toPandas()
        #pattern = re.compile(r"^\d{15}$")
        #summary_table = summary_table[summary_table["imei"].map(lambda x: bool(re.match(pattern, x)))]
        #sqlCtx = SQLContext(sc)
        #summary_table = sqlCtx.createDataFrame(summary_table)
	summary_table.rdd.map(tuple).saveAsPickleFile("/user/mjoys/user_vector/total_summary" + str(from_idx/50))
	print("Get!")
	end_time = time.time()
